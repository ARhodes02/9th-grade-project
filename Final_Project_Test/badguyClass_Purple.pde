class PurpleBadguy {
  ...
  public PurpleBadguy(float x, float y, float xSize, float ySize, float delay, float bulletSpeed, float life, float damage) {
    this.x = x;
    this.y = y;
    this.xSize = xSize;
    this.ySize = ySize;
  ...
  boolean faire() {
    ...
    if (purpleDelayControl <= 0) { 
      fill(35, 100, 75, 100);
      list.add(new bullet(x+xSize/2, y+ySize/2, px + psizeX/2, py + psizeY/2, bulletSpeed, 5, 5, 10, color(76, 100, 100, 100)));
